...
const CandidateTable: React.FC<CandidateTableProps> = ({ data, onStatusToggle }) => {
    ...
    <Table className="text-xl">
      ...
      <TableBody>
        {data.length === 0 ? (
          ...
          data.map((candidate) => (
            <TableRow key={candidate.id} className={!candidate.is_active ? "opacity-60" : ""}>
              <TableCell className="font-medium">{candidate.id}</TableCell>
              <TableCell className="cursor-pointer hover:text-primary hover:underline" 
                onClick={() => handleViewUser(candidate.id)}>
                {candidate.email}
              </TableCell>
              <TableCell className="text-center">{formatDate(candidate.created_at)}</TableCell>
              <TableCell className="text-center">
                <span className={`px-2 py-1 rounded-full text-xl ${candidate.is_verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {candidate.is_verified ? 'Verified' : 'Unverified'}
                </span>
              </TableCell>
              <TableCell className="text-center">
                <span className={`px-2 py-1 rounded-full text-xl ${candidate.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {candidate.is_active ? 'Approved' : 'Blocked'}
                </span>
              </TableCell>
              <TableCell className="flex justify-center gap-2">
                ...
                >
                  {loadingId === candidate.id ? (
                    ...
                    candidate.is_active ? 'Block' : 'Approve'
                  )}
                </Button>
              </TableCell>
            </TableRow>
